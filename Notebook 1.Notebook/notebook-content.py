...
def scholarship_prob(ses, region):
    return (F.when(ses == "Low", 0.35)
             .when(ses == "Medium", 0.18)
             .otherwise(0.06)) + (F.when(region.isin("Lagos", "Abuja"), -0.03).otherwise(0.0))
...
students = (spark.range(1, N_STUDENTS + 1)
    .withColumnRenamed("id", "student_id")
    .withColumn("r1", F.rand(RANDOM_SEED))
    .withColumn("r2", F.rand(RANDOM_SEED + 1))
    .withColumn("r3", F.rand(RANDOM_SEED + 2))
    .withColumn("r4", F.rand(RANDOM_SEED + 3))
    .withColumn("age", (F.floor(F.lit(14) + F.rand(RANDOM_SEED) * 17)).cast("int"))  # 14-30
...
students = students.withColumn(
    "scholarship_prob",
    scholarship_prob(F.col("socio_econ_status"), F.col("location"))
).withColumn(
    "scholarship",
    (F.rand(RANDOM_SEED + 4) < F.col("scholarship_prob")).cast("boolean")
...
base_activity = (students.select("student_id")
                 .crossJoin(weeks_df)
                 .withColumn("r_a", F.rand(RANDOM_SEED + 5))
                 .withColumn("r_b", F.rand(RANDOM_SEED + 6))
                 .withColumn("r_c", F.rand(RANDOM_SEED + 7)))

# Latent risk driver (static per student) combining proxy hardships
risk_base = (students
    .withColumn("risk_latent",
...
activity = (base_activity
    ...
    .withColumn("logins_per_week", F.greatest(F.lit(0), F.round(7 - 2*F.col("risk_latent") + F.rand(RANDOM_SEED + 8) * 4).cast("int")))
    .withColumn("avg_session_duration_min", F.round(20 + (1 - F.col("risk_latent")) * 25 + F.rand(RANDOM_SEED + 9) * 15, 1))
    .withColumn("video_completion_rate", F.round(F.least(F.lit(1.0), F.greatest(F.lit(0.0), 0.35 + (1 - 0.3*F.col("risk_latent")) * 0.45 + (F.rand(RANDOM_SEED + 10) - 0.5) * 0.2)), 3))
    .withColumn("discussion_participation", F.greatest(F.lit(0), F.round((1 - 0.5*F.col("risk_latent")) * 5 + F.rand(RANDOM_SEED + 11) * 3).cast("int")))
    .withColumn("connectivity_issues", F.greatest(F.lit(0), F.round(0.8 + 0.8*F.col("risk_latent") + F.rand(RANDOM_SEED + 12) * 2).cast("int")))
)
...
performance = (perf_base
    ...
    .withColumn("assignment_submission_rate", F.round(F.least(F.lit(1.0), F.greatest(F.lit(0.0), 0.85 - 0.2*F.col("risk_latent") + (F.rand(RANDOM_SEED + 13)-0.5) * 0.2)), 3))
    .withColumn("avg_score", F.round(F.least(F.lit(100.0), F.greatest(F.lit(0.0), 65 + (1 - F.col("risk_latent")) * 20 + (F.rand(RANDOM_SEED + 14) - 0.5) * 25)), 1))
)
...
attendance = (students.select("student_id").crossJoin(weeks_df)
    .join(risk_base, "student_id")
    .withColumn("classes_scheduled", F.lit(5))
    .withColumn("classes_attended", F.greatest(F.lit(0), F.least(F.col("classes_scheduled"), F.round(4.3 - 0.8*F.col("risk_latent") + F.rand(RANDOM_SEED + 15) * 1.5).cast("int")))
...
surveys = (students.select("student_id").crossJoin(survey_weeks)
    .join(risk_base, "student_id")
    .withColumn("motivation_level", F.greatest(F.lit(1), F.least(F.lit(10), F.round(7 - 2*F.col("risk_latent") + F.rand(RANDOM_SEED + 16) * 2).cast("int"))))
    .withColumn("stress_level", F.greatest(F.lit(1), F.least(F.lit(10), F.round(4 + 2*F.col("risk_latent") + F.rand(RANDOM_SEED + 17) * 3).cast("int"))))
    .withColumn("peer_interaction", F.greatest(F.lit(0), F.round((1 - 0.4*F.col("risk_latent")) * 10 + F.rand(RANDOM_SEED + 18) * 5).cast("int")))
)
...
activity_agg = (spark.table(f"{BRONZE_SCHEMA}.student_activity_logs")
    .groupBy("student_id")
    .agg(
...
perf_agg = (spark.table(f"{BRONZE_SCHEMA}.student_academic_performance")
    .groupBy("student_id")
    .agg(
...
attendance_agg = (spark.table(f"{BRONZE_SCHEMA}.student_attendance")
    .groupBy("student_id")
    .agg(
...
behavior_agg = (spark.table(f"{BRONZE_SCHEMA}.student_behavioral_surveys")
    .groupBy("student_id")
    .agg(
...
perf_weeks = spark.table(f"{BRONZE_SCHEMA}.student_academic_performance")\
    .groupBy("student_id", "week").agg(F.avg("avg_score").alias("week_avg_score"))
...
trend = (first4.join(last4, "student_id", "outer")
...
risk_signal = (
    ...
    1.0*(F.col("login_volatility").na_fill(0)) +
...
gold = (silver_join
    .withColumn("engagement_score", F.round(0.4*F.col("avg_logins") + 0.6*F.col("avg_video_completion")*10, 3))
    .withColumn("consistency_score", F.round(10/(1+F.col("login_volatility")), 3))
    .withColumn("dropout_probability", p_dropout)
    .withColumn("dropout_label", (F.col("dropout_probability") > 0.35).cast("int"))
    .select(
        ...
        F.round("dropout_probability", 4).alias("dropout_probability"),
        "dropout_label"
    )
)
...
# Optional: create SQL view for DA/DS convenience
spark.sql(f"""
